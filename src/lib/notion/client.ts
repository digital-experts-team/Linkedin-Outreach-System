...
/**
 * Creates a new LinkedIn post in the Notion Content Hub database.
 */
export async function createLinkedInPostInNotion(
  ...
  try {
    const postName = (input.name || '').trim() || (input.hook ? input.hook.slice(0, 60) : 'Untitled Draft Post');
    const propertiesToSet: Record<string, any> = {
      ...
      'Full Copy': {
        rich_text: createRichTextChunks(input.fullCopy || ''),
      },
      Status: {
        select: {
          name: input.status || 'Draft',
        },
      },
    };

    if (input.hook) {
      propertiesToSet['Hook'] = {
        rich_text: createRichTextChunks(input.hook),
      };
    }

    if (input.vertical) {
      propertiesToSet['Vertical'] = {
        select: {
          name: input.vertical,
        },
      };
    ...
    if (input.ctaKeyword) {
      propertiesToSet['CTA Keyword'] = {
        rich_text: createRichTextChunks(input.ctaKeyword),
      };
    }

    if (input.scheduledDate) {
      propertiesToSet['Scheduled Date'] = {
        date: {
    ...
    if (input.images && input.images.length > 0) {
      propertiesToSet['Image'] = {
        files: formatImagesForNotion(input.images),
      };
    }
    ...
    const post = normalizeNotionPostPage(createdPage);
    ...
    return {
      ...
      error: {
        code: 'FETCH_ERROR',
        message: error?.message || 'Failed to create post in Notion Content Hub',
      },
    };
  }
}
